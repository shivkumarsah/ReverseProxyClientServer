...
class ServiceProviderTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testShouldRegister()
    {
        ...
        $sp = m::mock(
                        ...
                        'registerUserValidator]', ['something']
        );
         ...
         */
        $sp->shouldReceive(
                        'registerRepository', 'registerConfide', 'registerCommands', 'registerPasswordService', 'registerLoginThrottleService', 'registerUserValidator'
                )
                ->once();
        /*
          |------------------------------------------------------------
...
// kind of object.
                function ($name, $closure) use ($test, $app) {
                    ...
                    $test->assertInstanceOf(
                            'Zizaco\Confide\EloquentRepository', $closure($app)
                    );
                }
        );
        /*
...
// kind of object.
                function ($name, $closure) use ($test, $app) {
                    ...
                    $test->assertInstanceOf(
                            'Zizaco\Confide\EloquentPasswordService', $closure($app)
                    );
                }
        );
        /*
...
// kind of object.
                function ($name, $closure) use ($test, $app) {
                    ...
                    $test->assertInstanceOf(
                            'Zizaco\Confide\CacheLoginThrottleService', $closure($app)
                    );
                }
        );
        /*
...
// kind of object.
                function ($name, $closure) use ($test, $app) {
                    ...
                    $test->assertInstanceOf(
                            'Zizaco\Confide\UserValidator', $closure($app)
                    );
                }
        );
        /*
...
// kind of object.
                function ($name, $closure) use ($test, $app) {
                    ...
                    $test->assertInstanceOf(
                            'Zizaco\Confide\Confide', $closure($app)
                    );
                }
        );
        /*
...
// object.
                function ($name, $closure) use ($test, $app) {
                    ...
                    $test->assertInstanceOf(
                            $shouldBe[$name], $closure($app)
                    );
                }
        );
        $sp->shouldReceive('commands')
                ->with(
                        'command.confide.controller', 'command.confide.routes', 'command.confide.migration'
        );
        /*
          |------------------------------------------------------------
          | Assertion
