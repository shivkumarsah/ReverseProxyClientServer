...
class UserValidatorTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testShouldValidatePassword()
    {
        ...
        $validator->shouldReceive('attachErrorMsg')
                ->atLeast(1)
                ->with(
                        m::any(), 'confide::confide.alerts.password_confirmation', 'password_confirmation'
        );
        /*
          |------------------------------------------------------------
          | Assertion
...
// be hidden from toArray method.
        ...
        $laravelValidator->shouldReceive('make')
                ->with(
                        ['username' => 'foo', 'password' => 'secret'], $validator->rules['create']
                )
                ->once()->andReturn($laravelValidator);
        $laravelValidator->shouldReceive('make')
                ->with(
                        ['username' => 'bar', 'password' => 'p@ss'], $validator->rules['create']
                )
                ->once()->andReturn($laravelValidator);
