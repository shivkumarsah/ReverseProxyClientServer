...
class UserRepository
{
    ...
    {   
        $x  = app();
        $correctPassword =  $x['hash']->check(
            isset($input['oldpassword']) ? $input['oldpassword'] : false,$input['pass']
        );
        
        if($correctPassword === true){ 
            return array("status"=>true, "responseMessage"=>"Password changed successfully");
        }else{
           return array("status"=>false, "responseMessage"=>"Invalid old password");
        }
    }
